...
--*/
{
  ...
  ReadOnly = IFile->ReadOnly || (DirEnt->Entry.Attributes & EFI_FILE_READ_ONLY);
  //
  // if a zero time is specified, then the original time is preserved
  //
  if (EfiCompareMem (&ZeroTime, &NewInfo->CreateTime, sizeof (EFI_TIME)) != 0) {
  ...
  if (EfiCompareMem (&ZeroTime, &NewInfo->ModificationTime, sizeof (EFI_TIME)) != 0) {
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (IsSet) {
      if (Volume->ReadOnly) {
        ...
        if (EfiCompareGuid (Type, &gEfiFileSystemInfoGuid)) {
