...
--*/
{
  ...
  if (--EntryCount > 0) {
    ...
    //
    EfiSetMem (LfnBuffer, sizeof (CHAR16) * LFN_CHAR_TOTAL * EntryCount, 0xff);
    EfiStrCpy (LfnBuffer, DirEnt->FileString);
    ...
    for (LfnOrdinal = 1; LfnOrdinal <= EntryCount; LfnOrdinal++) {
      ...
      EfiCopyMem (LfnEntry.Name1, LfnBufferPointer, sizeof (CHAR16) * LFN_CHAR1_LEN);
      LfnBufferPointer += LFN_CHAR1_LEN;
      EfiCopyMem (LfnEntry.Name2, LfnBufferPointer, sizeof (CHAR16) * LFN_CHAR2_LEN);
      LfnBufferPointer += LFN_CHAR2_LEN;
      EfiCopyMem (LfnEntry.Name3, LfnBufferPointer, sizeof (CHAR16) * LFN_CHAR3_LEN);
...
--*/
{
  ...
  if (EfiStrCmp (FileString, L".") == 0 || EfiStrCmp (FileString, L"..") == 0) {
    return TRUE;
  }
...
--*/
{
  ...
  do {
    ...
    EfiCopyMem (LfnBufferPointer, LfnEntry.Name1, sizeof (CHAR16) * LFN_CHAR1_LEN);
    LfnBufferPointer += LFN_CHAR1_LEN;
    EfiCopyMem (LfnBufferPointer, LfnEntry.Name2, sizeof (CHAR16) * LFN_CHAR2_LEN);
    LfnBufferPointer += LFN_CHAR2_LEN;
    EfiCopyMem (LfnBufferPointer, LfnEntry.Name3, sizeof (CHAR16) * LFN_CHAR3_LEN);
    LfnBufferPointer += LFN_CHAR3_LEN;
    LfnOrdinal++;
  ...
  *LfnBufferPointer = 0;
  ...
  DirEnt->FileString = EfiLibAllocateCopyPool (EfiStrSize (LfnBuffer), LfnBuffer);
}
...
--*/
{
  ...
  for (;;) {
    ...
    if (((UINT8)Entry.FileName[0] != DELETE_ENTRY_MARK) && 
        (Entry.Attributes & FAT_ATTRIBUTE_VOLUME_ID) == 0) {
      //
  ...
  if (Entry.FileName[0] != EMPTY_ENTRY_MARK) {
    ...
    EfiCopyMem (&DirEnt->Entry, &Entry, sizeof (FAT_DIRECTORY_ENTRY));
...
--*/
{
  ...
  NameSize    = EfiStrSize (DirEnt->FileString);
  ...
  if (*BufferSize >= ResultSize) {
    ...
    EfiZeroMem (&FatLastAccess.Time, sizeof (FatLastAccess.Time));
    EfiCopyMem (&FatLastAccess.Date, &Entry->FileLastAccess, sizeof (FatLastAccess.Date));
    ...
    EfiCopyMem ((CHAR8 *) Buffer + Size, DirEnt->FileString, NameSize);
  }
  ...
  if (DirEnt == NULL) {
    ...
    while (!ODir->EndOfDir) {
      ...
      if (DirEnt != NULL) {
        ...
        if (PossibleShortName && EfiCompareMem (File8Dot3Name, DirEnt->Entry.FileName, FAT_NAME_LEN) == 0) {
          break;
        }
      }
    }
...
--*/
{
  ...
  if (EfiStrCmp (FileString, L".") == 0) {
    //
    // "." entry
    //
    File8Dot3Name[0] = '.';
    FatCloneDirEnt (DirEnt, OFile->DirEnt);
  } else if (EfiStrCmp (FileString, L"..") == 0) {
    //
    // ".." entry
    //
    File8Dot3Name[0]  = '.';
    ...
    if (FatCheckIs8Dot3Name (FileString, File8Dot3Name)) {
      ...
      DirEnt->EntryCount += LFN_ENTRY_NUMBER ((UINT8) EfiStrLen (FileString));
    }
  }
}
...
--*/
{
  ...
  if (LabelDirEnt.Invalid) {
    ...
    //
    EfiZeroMem (&LabelDirEnt, sizeof (FAT_DIRENT));
  ...
  EfiSetMem (LabelDirEnt.Entry.FileName, FAT_NAME_LEN, ' ');
...
--*/
{
  ...
  DirEnt->Signature   = FAT_DIRENT_SIGNATURE;
  DirEnt->FileString  = EfiLibAllocateCopyPool (EfiStrSize (FileName), FileName);
  if (DirEnt->FileString == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
    goto Done;
  }
...
--*/
{
  ...
  if (DirEnt->OFile == NULL) {
    ...
    if (Parent != NULL) {
      ...
      OFile->FullPathLen = Parent->FullPathLen + 1 + EfiStrLen (DirEnt->FileString);
