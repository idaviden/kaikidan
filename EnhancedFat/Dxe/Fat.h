...
#define Log2                      (UINT8)HighBitSet64
...
typedef struct _FAT_DIRENT {
  ...
  EFI_LIST_ENTRY      Link;                   // Connection of every directory entry
...
typedef struct _FAT_ODIR {
  ...
  EFI_LIST_ENTRY      *CurrentCursor;         // Current directory entry pointer
  EFI_LIST_ENTRY      ChildList;              // List of all directory entries
  BOOLEAN             EndOfDir;               // Indicate whether we have reached the end of the directory
  EFI_LIST_ENTRY      DirCacheLink;           // Linked in Volume->DirCacheList when discarded
