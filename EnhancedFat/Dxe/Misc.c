...
--*/
{
  if (DirEnt->FileString != NULL) {
    gBS->FreePool (DirEnt->FileString);
  }
...
--*/
{
  ...
  if (Volume->CacheBuffer != NULL) {
    gBS->FreePool (Volume->CacheBuffer);
  }
  //
...
Arguments: 
  ...
  Str1                  - The first string which will be compared.
  Str2                  - The second string which will be compared.
...
{
  ASSERT (gUnicodeCollationInterface != NULL);
  return gUnicodeCollationInterface->StriColl (
...
{
  ASSERT (gUnicodeCollationInterface != NULL);
  gUnicodeCollationInterface->FatToStr (
                                gUnicodeCollationInterface,
                                FatSize,
                                Fat,
                                String
...
{
  ASSERT (gUnicodeCollationInterface != NULL);
  return gUnicodeCollationInterface->StrToFat (
                                       gUnicodeCollationInterface,
                                       String,
                                       FatSize,
                                       Fat
