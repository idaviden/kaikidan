...
#define MCAST_HASH_BYTES  8
  ...
  return SetCurrentMacAddress (LanDriver, &SnpMode->CurrentAddress);
}
...
)
{
  ...
  return SetCurrentMacAddress (LanDriver, &Snp->Mode->CurrentAddress);
}
  ...
  )
{
  ...
  if (TxBuff != NULL) {
    *TxBuff = LanDriver->TxBuffer;
    LanDriver->TxBuffer = NULL;
  }

  // Update the media status
  Snp->Mode->MediaPresent = CheckLinkStatus (LanDriver);
  ...
  )
{
  ...
  MmuOperation (LanDriver, MMUCR_OP_TX_PUSH);
  ...
  LanDriver->TxBuffer = BufAddr;

  // Dump the packet header
#if LAN91X_PRINT_PACKET_HEADERS
  Ptr = BufAddr;
  DEBUG((EFI_D_ERROR, "LAN91X:SnpTransmit()\n"));
  DEBUG((EFI_D_ERROR, "  HdrSize: %d, SrdAddr: %p, Length: %d, Last byte: %02x\n",
