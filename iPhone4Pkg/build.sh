...
BUILD_ROOT=$WORKSPACE/Build/iPhone4/"$TARGET"_"$TARGET_TOOLS"
GENERATE_IMAGE=$WORKSPACE/iPhone4Pkg/Tools/generate_image
...
if [[ $TARGET == RELEASE ]]; then
  build -p $WORKSPACE/iPhone4Pkg/iPhone4Pkg.dsc -a ARM -t $TARGET_TOOLS -b $TARGET -D DEBUG_TARGET=RELEASE ${2:-} ${3:-} ${4:-} ${5:-} ${6:-} ${7:-} ${8:-}
else
  build -p ${WORKSPACE:-}/iPhone4Pkg/iPhone4Pkg.dsc -a ARM -t $TARGET_TOOLS -b $TARGET ${1:-} ${2:-} ${3:-} ${4:-} ${5:-} ${6:-} ${7:-} ${8:-}
fi
...
do
  if [[ $arg == clean ]]; then
    ...
    make -C $WORKSPACE/iPhone4Pkg/Tools clean
    exit
...
if [[ ! -e $GENERATE_IMAGE ]];
then
  make -C $WORKSPACE/iPhone4Pkg/Tools
fi

echo Creating debugger scripts
process_debug_scripts $WORKSPACE/iPhone4Pkg/Debugger_scripts
